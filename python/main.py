...
def main():
    with sync_playwright() as playwright:
        ...
        month_country_to_fruit_dict = dict()
        for fruit in fruit_to_month_country_dict:
            for month_country in fruit_to_month_country_dict[fruit]:
                month_country_to_fruit_dict.setdefault(month_country, []).append(fruit)
        with open('dist/month-country-to-fruit.json', 'w') as file:
            file.write(f'const fruitObject = {json.dumps(month_country_to_fruit_dict)};')
        vegetable_child_list = page.query_selector('#Vegetable > .fvgrid').query_selector_all('xpath=child::*')
        vegetable_to_month_country_dict = dict()
        ...
        month_country_to_vegetable_dict = dict()
        for vegetable in vegetable_to_month_country_dict:
            for month_country in vegetable_to_month_country_dict[vegetable]:
                month_country_to_vegetable_dict.setdefault(month_country, []).append(vegetable)
        with open('dist/month-country-to-vegetable.json', 'w') as file:
            file.write(f'const vegetableObject = {json.dumps(month_country_to_vegetable_dict)};')
        browser.close()
